...
object BasicTypes extends App {
   ...
   * Exercise
   *
   * Filter the cars DF by a list of car names obtained by an API call (val ls = List(1, 2, 3))
   * Versions:
   *   - contains
   */

  val ls = List("chevrolet", "ford", "dodge")
  val regexCarsFromList = ls.mkString("|")
  val americanCarsDF = carsDF.select(
    col("Name"),
    regexp_extract(col("Name"), regexCarsFromList, 0).as("regex_extract")
  ).where(col("regex_extract") =!= "").drop("regex_extract")
